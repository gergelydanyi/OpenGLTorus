...
constexpr auto tmr_Tick = 1;
int TickTime = 2;

const wchar_t* className = L"OpenGL";
const wchar_t* windowName = L"Just a torus";
int winX = 300, winY = 200;
int winWidth = 550, winHeight = 500;
int mX, mY, mXp, mYp;
...
drawTorus(void)
{
	...
	for (i = 0; i < numMajor; ++i) {
		...
		GLfloat x0 = (GLfloat)cos(a0)*1.0;
		GLfloat y0 = (GLfloat)sin(a0)*1.0;
		GLfloat x1 = (GLfloat)cos(a1)*1.0;
		GLfloat y1 = (GLfloat)sin(a1)*1.0;

		if (i == 1) {
			glColor3fv(colors[3].diff);
			glMaterialiv(GL_FRONT, GL_COLOR_INDEXES, colors[3].indexes);
		}
		else if (i & 1) {
...
setProjection(void)
{
	...
	*/
	if (winWidth > winHeight) {
		GLdouble aspect = (GLfloat)winWidth / (GLfloat)winHeight;
		glFrustum(-0.5F * aspect, 0.5F * aspect, -0.5F, 0.5F, 1.0F, 3.0F);
	}
	else {
		GLdouble aspect = (GLfloat)winHeight / (GLfloat)winWidth;
...
init(void)
{
	...
	glClearColor(colors[2].diff[0], colors[2].diff[1], colors[2].diff[2], 0.0F);
...
doRedraw(void)
{
	static GLfloat x0 = -90, x = 0, y = -30, z=0;
	static GLfloat amp = 335.0F, frek = 0.004F;

	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

	glPushMatrix();
	glTranslatef((GLfloat)sin(z) * 0.5F, 0.0F, (GLfloat)cos(z) * 0.5F-0.5F);
...
setupDIB(HDC hDC)
{
	...
	if (hBitmap == NULL) {
		(void)MessageBox(WindowFromDC(hDC),
			L"Failed to create DIBSection.",
			L"OpenGL application error",
			MB_ICONERROR | MB_OK);
		exit(1);
	}
...
setupPixelFormat(HDC hDC)
{
	...
	if (SelectedPixelFormat == 0) {
		(void)MessageBox(WindowFromDC(hDC),
			L"Failed to find acceptable pixel format.",
			L"OpenGL application error",
			MB_ICONERROR | MB_OK);
		exit(1);
	}
...
//		WS_EX_LAYERED |
	...
	SetTimer(hWnd, tmr_Tick, TickTime, NULL);
	/*
